function [sol,b,sv,obj] = primal_svm(linear,X,Y,lambda,opt)
...
% Last modified 25/08/2006  
  ...
  if  linear 
    ...
    if ~opt.cg
      [sol,sv,obj] = primal_svm_linear   (X,Y,lambda,opt);
    else
...
function  [w,sv, obj] = primal_svm_linear(X,Y,lambda,opt) 
% -------------------------------
% Train a linear SVM using Newton 
% -------------------------------
  %global X;
  [n,d] = size(X);
  if ~isempty(opt.w0)&&size(opt.w0,1)==d,
      w=[opt.w0;1];
  else
      w = zeros(d+1,1); % The last component of w is b.
      fprintf('no intial value for w\n');
  end
  iter = 0;
  out = 1-Y.*(X*w(1:end-1)); % Vector containing 1-Y.*(X*w)
  ...
  while 1
    ...
    [obj, grad, sv] = obj_fun_linear(w,X,Y,lambda,out);      
    ...
    [t,out] = line_search_linear(w,step,out,X,Y,lambda);
...
function [obj, grad, sv] = obj_fun_linear(w,X,Y,lambda,out)
...
function y = hess_vect_mult(X,w,sv,lambda)
...
function [t,out] = line_search_linear(w,d,out,X,Y,lambda) 
